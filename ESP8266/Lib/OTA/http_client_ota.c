...
static unsigned char *Version_output;
static uint16_t *Version_dowload;
static char *SHA256_str;
static char *Version_str;
static char *SHA256_wrt_ptr;
static char *Version_wrt_ptr;
...
static unsigned int Version_check_callback(char *buf, uint16_t size)
{
    int Current_Version_Size;
    
    // Check that str does not contains other streing with SHA256
    if (size > SHA256_SIZE_STR)
        size = SHA256_SIZE_STR;
    
    Current_Version_Size = Version_wrt_ptr - (char *) Version_str;
    
    if (!(Current_Version_Size > SHA256_SIZE_STR)) {
        memcpy(Version_wrt_ptr, buf, size);
        Version_wrt_ptr += size;
    }
    return 1;
}
...
OTA_err ota_update(ota_info *ota_info_par)
{
    ...
    if (ota_inf->version_path != NULL) {
        Version_output  = malloc(SHA256_SIZE_BIN);
        Version_dowload = malloc(SHA256_SIZE_BIN);
        Version_str     = malloc(SHA256_SIZE_STR + 1);
        Version_wrt_ptr = Version_str;
        Version_str[SHA256_SIZE_STR] = '\0';
    }
    ...
    http_inf.path           = ota_inf->version_path;
    http_inf.final_cb       = Version_check_callback;
    http_inf.buffer_full_cb = Version_check_callback;
    
    memset(Version_dowload, 0, 10);
    memset(Version_str, 0, 20);
    DEBUG_PRINT("Starting to get version");
    vTaskDelayMs(20);
    err = HttpClient_dowload(&http_inf);
    // Check if dowload success
    if (err != HTTP_OK)
        goto dealloc_all;
    DEBUG_PRINT("version %s",Version_str);
    if(!isNeedUpdate(Version_str)){
        err = 101;
        DEBUG_PRINT("do not need update");
//        free(http_inf.buffer);
//        if (ota_inf->version_path != NULL) {
//            free(SHA256_str);
//            free(SHA256_output);
...
//        }
    ...
    if (ota_inf->sha256_path != NULL) {
        ...
        DEBUG_PRINT("Sha download %s",SHA256_dowload);
        DEBUG_PRINT("Sha str %s",SHA256_str);
    ...
    // Ping Wdog
    vTaskDelayMs(20);
    //DEBUG_PRINT("start update Dowload Firmaware");
    // Dowload Firmaware
    ...
    err = HttpClient_dowload(&http_inf);
    //vPortExitCritical();
    DEBUG_PRINT("end update Dowload Firmaware");
    if (err != HTTP_OK)
        goto dealloc_all;
    ...
    DEBUG_PRINT("start flash Firmaware");
    // Ping watch DOG
    vTaskDelayMs(5);
    {
